# -*- coding: utf-8 -*-
"""
Created on Wed May 24 18:21:12 2023

@author: Lenovo
"""

#Veri Setimiz kaggle dan alınmıştır.

#Arayüz tasarımı (GUI) için gerekli olan kütüphaneler
import tkinter as tk
from tkinter import ttk
from tkinter import filedialog
from tkinter import messagebox

#Görsellerimizi arayüze aktarmak için gerekli olan kütüphane
...
skin_df.head() # ilk 5 sütunu getirir.
skin_df.info() # bize genel bilgi verir . örneğin kaç adet resim bulunduğu , sütun sayısı gibi
sns.countplot(x = "dx" , data=skin_df) #dx sütununda bulunan veriyi görselleştirdik.


#%%
data_folder_name = "HAM10000_images_part_1/" #Resimlerimin bulunduğu dosyanın adı
ext = ".jpg" #Resimlerimin türü
skin_df["path"] = [data_folder_name + i + ext for i in skin_df["image_id"]] #Resimlerin adresini "path" adını verdiğimiz kolanda tutuyoruz.
skin_df["image"] = skin_df["path"].map(lambda x: np.asanyarray(Image.open(x).resize((100,75))))
plt.imshow(skin_df["image"][0])

#Class larımızı kodlayalım.Örneğin nv=5, akiec=0 ......
skin_df["dx_idx"] = pd.Categorical(skin_df["dx"]).codes

#Verilerimizi pickle dosyasına kaydettik.
skin_df.to_pickle("skin_df.pkl")


#%%
#Pickle dosyamızı okuyalım
skin_df = pd.read_pickle("skin_df.pkl")

#%% stardardization

x_train = np.asanyarray(skin_df["image"].tolist())
...
#%% Kullanıcı Arayüzü oluşturma kodları (GUI) TASARIM KISMI
...
## frames(Tasarımımızdaki çerçeveler)
...
def openImage():
    ...
    count += 1
    if count != 1:
        messagebox.showinfo(title = "UYARIII", message = "Sadece 1 adet resim secebilirsin")
    else:
...
def classification():
    ...
    if img_name != "" and models.get() != "":
        ...
        messagebox.showinfo(title = "UYARII", message = "Lutfen model seciniz...")
